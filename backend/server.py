        ...
        for emp in employees:
            ...
            for record in all_attendance:
                ...
                if status == "absent":
                    ...
                    if late_minutes <= 15:
                        if late_count <= 4:
                            ...
                            rule_applied = f"Late >4 times: Cumulative deduction"
                            minute_rate = employee_salary / (30 * 8 * 60)
                            day_deduction = minute_rate * late_minutes
                            late_deduction += day_deduction
                            deduction_type = "late"
                            note = f"تأخير {late_minutes} دقيقة - بعد الحد المجاني"
                    
                    elif 15 < late_minutes <= 20:
                        rule_applied = "Late >15 min: Actual time deduction"
                        minute_rate = employee_salary / (30 * 8 * 60)
                        day_deduction = minute_rate * late_minutes
                        late_deduction += day_deduction
                        deduction_type = "late"
                    ...
                    elif 20 < late_minutes <= 120:  # 20 min to 2 hours
                        if late_minutes <= 60:  # Up to 1 hour
                            rule_applied = "Late >20 min: Actual time"
                            minute_rate = employee_salary / (30 * 8 * 60)
                            day_deduction = minute_rate * late_minutes
                            deduction_type = "late"
                            note = f"تأخير {late_minutes} دقيقة"
                        else:  # 1-2 hours
                            rule_applied = "Late 1-2 hours: Half day"
                            daily_rate = employee_salary / 30
                            day_deduction = daily_rate / 2
                            deduction_type = "half_day"
                            note = f"تأخير {late_minutes} دقيقة - خصم نصف يوم"
                        
                        late_deduction += day_deduction
                    
                    else:  # > 2 hours
                        rule_applied = "Late >2 hours: Full day"
                        daily_rate = employee_salary / 30
                        day_deduction = daily_rate
                        late_deduction += day_deduction
                        deduction_type = "full_day"
                        note = f"تأخير {late_minutes} دقيقة - خصم يوم كامل"
                
                # Add early leave deduction (if any)
                if early_leave_minutes > 0:
                    minute_rate = employee_salary / (30 * 8 * 60)
                    early_leave_ded = minute_rate * early_leave_minutes
                    day_deduction += early_leave_ded
                    late_deduction += early_leave_ded  # Count as late deduction
                    note += f" | انصراف مبكر {early_leave_minutes} دقيقة"
                ...
                if check_in and check_out:
                    try:
                        ...
                        total_work_minutes = int((check_out_time - check_in_time).total_seconds() / 60)
                    except:
                        total_work_minutes = 0
                ...
                daily_breakdown.append({
                    ...
                    "late_minutes": late_minutes,
                    "early_leave_minutes": early_leave_minutes,
                    "deficit_minutes": late_minutes + early_leave_minutes,
                    "working_hours": working_hours,
                    "rule_applied": rule_applied,
                    "deduction_type": deduction_type,
                    "deduction_amount": round(day_deduction, 2),
            ...
            if total_employee_deduction > 0 or len(daily_breakdown) > 0:
                results.append({
                    ...
                    "daily_records": daily_breakdown  # ✅ Full daily details for custom period
                })
                total_deductions += total_employee_deduction
...
async def calculate_deductions_flexible(
