from fastapi import FastAPI, APIRouter, HTTPException, Depends, status, UploadFile, File, Form, Response, Query
...
📅 التاريخ: {get_uae_now()  # UAE timezone.strftime('%Y-%m-%d %H:%M')}
...
يمكنك الآن استخدام هذا المبلغ في مصروفاتك."""
    ...
    notification = Notification(
        ...
        type="success",
        priority="normal",
        sent_at=get_uae_now()  # UAE timezone
    )
...
async def get_daily_attendance(
    ...
    if not date:
        date = get_uae_now()  # UAE timezone.date().isoformat()
...
async def check_in_attendance(
    ...
    check_in_time = get_uae_now()  # UAE timezone
...
async def check_out_attendance(
    ...
    check_out_time = get_uae_now()  # UAE timezone
...
async def recompute_attendance(
    ...
    if not month:
        month = get_uae_now()  # UAE timezone.strftime('%Y-%m')
...
async def export_payroll_pdf(
    ...
    try:
        ...
        info_text = f"تاريخ الإصدار: {get_uae_now()  # UAE timezone.strftime('%Y-%m-%d %H:%M')}"
...
async def export_payroll_excel(
    ...
    try:
        ...
        info_cell = ws['A2']
        info_cell.value = f"تاريخ الإصدار: {get_uae_now()  # UAE timezone.strftime('%Y-%m-%d %H:%M')}"
...
async def create_user(user: UserCreate, current_user: User = Depends(get_admin_user)):
    ...
    user_dict["created_at"] = get_uae_now()  # UAE timezone
...
async def update_attendance(attendance_id: str, update_data: dict, current_user: User = Depends(get_admin_user)):
    ...
    if check_in_updated or check_out_updated or new_status:
        ...
        update_fields["modified_at"] = get_uae_now()  # UAE timezone
...
async def edit_absence_record(attendance_id: str, attendance_data: dict, current_user: User = Depends(get_super_admin_user)):
    ...
    update_data = {
        ...
        "modified_at": get_uae_now()  # UAE timezone,
...
async def process_daily_absences(date_data: dict, current_user: User = Depends(get_super_admin_user)):
    ...
    if not target_date:
        target_date = get_uae_now()  # UAE timezone.strftime("%Y-%m-%d")
...
async def get_missing_employees_today(current_user: User = Depends(get_admin_user)):
    ...
    today = get_uae_now()  # UAE timezone.strftime("%Y-%m-%d")
...
async def reject_leave(leave_id: str, rejection_data: dict = None, current_user: User = Depends(get_admin_user)):
    ...
    update_data = {
        ...
        "rejected_at": get_uae_now()  # UAE timezone,
        "admin_notes": notes
    }
...
async def export_report(report_type: str, start_date: str, end_date: str, format: str = "excel", current_user: User = Depends(get_admin_user)):
    ...
    if format == "excel":
        ...
        period_cell = ws['A4']
        period_cell.value = f"الفترة: {start_date} إلى {end_date} | عدد السجلات: {len(report_data)} | تاريخ الإنشاء: {get_uae_now()  # UAE timezone.strftime('%Y-%m-%d %H:%M')}"
        ...
        period_info = Paragraph(f"الفترة: {start_date} إلى {end_date}<br/>عدد السجلات: {len(report_data)}<br/>تاريخ الإنشاء: {get_uae_now()  # UAE timezone.strftime('%Y-%m-%d %H:%M')}", info_style)
...
async def export_report(report_type: str, month: str, format: str = "excel", current_user: User = Depends(get_admin_user)):
    ...
    if format == "excel":
        ...
        ws.merge_cells('A3:G3')
        period_cell = ws['A3']
        period_cell.value = f"Period: {start_date} to {end_date.split('-')[0]}-{end_date.split('-')[1]}-{int(end_date.split('-')[2])-1:02d} | Generated on: {get_uae_now()  # UAE timezone.strftime('%Y-%m-%d %H:%M')}"
        ...
        # Period info
        period_info = Paragraph(f"Period: {start_date} to {end_date.split('-')[0]}-{end_date.split('-')[1]}-{int(end_date.split('-')[2])-1:02d}<br/>Generated on: {get_uae_now()  # UAE timezone.strftime('%Y-%m-%d %H:%M')}", info_style)
        story.append(period_info)
        story.append(Spacer(1, 20))
...
async def export_payroll(month: str, format: str = "excel", current_user: User = Depends(get_admin_user)):
    ...
    if format == "excel":
        ...
        date_cell = ws['A4']
        date_cell.value = f"Generated on: {get_uae_now()  # UAE timezone.strftime('%Y-%m-%d %H:%M:%S')} UAE Time - تم الإنشاء في الوقت الإماراتي"
        ...
        ws.merge_cells('A4:G4')
        period_cell = ws['A4']
        period_cell.value = f"الشهر: {month} | عدد الموظفين: {len(payroll_data)} | تاريخ الإنشاء: {get_uae_now()  # UAE timezone.strftime('%Y-%m-%d %H:%M')}"
        period_cell.font = Font(name="Arial", size=10, color="555555")
        period_cell.fill = PatternFill(start_color="F8F9FA", end_color="F8F9FA", fill_type="solid")
        period_cell.alignment = Alignment(horizontal="center", vertical="center")
        ws.row_dimensions[4].height = 25
        ...
        # Period info
        period_info = Paragraph(f"الشهر: {month}<br/>عدد الموظفين: {len(payroll_data)}<br/>تاريخ الإنشاء: {get_uae_now()  # UAE timezone.strftime('%Y-%m-%d %H:%M')}", info_style)
        story.append(period_info)
        story.append(Spacer(1, 30))
    ...
    try:
        ...
        for penalty in penalties:
            penalty_record = {
                ...
                "applied_at": get_uae_now()  # UAE timezone,
                "status": "applied"
            }
    ...
    try:
        ...
        for log in recent_logs:
            formatted_logs.append({
                ...
                "error": log.get("error", ""),
                "time_ago": get_time_ago(log.get("timestamp", get_uae_now()  # UAE timezone))
            })
...
async def create_manual_backup(current_user: User = Depends(get_super_admin_user)):
    ...
    try:
        ...
        # Create timestamp
        timestamp = get_uae_now()  # UAE timezone.strftime("%Y%m%d_%H%M%S")
        ...
        if result.returncode == 0:
            ...
            return {
                ...
                "created_at": get_uae_now()  # UAE timezone.strftime("%Y-%m-%d %H:%M:%S")
            }
    ...
    try:
        ...
        for record in late_attendance:
            ...
            existing_notification = await db.messages.find_one({
                ...
                "created_at": {"$gte": get_uae_now()  # UAE timezone.replace(hour=0, minute=0, second=0, microsecond=0)}
            })
...
📅 تاريخ التطبيق: {get_uae_now()  # UAE timezone.strftime('%Y-%m-%d')}
    ...
    query = {
        "$and": [
            ...
            {"is_active": True},
            {
                "$or": [
                    {"expires_at": None},
                    {"expires_at": {"$gt": get_uae_now()  # UAE timezone}}
                ]
            }
        ]
    }
...
async def get_automation_status(current_user: User = Depends(get_super_admin_user)):
    ...
    try:
        ...
        recent_messages = await db.messages.find({
            ...
            "created_at": {"$gte": get_uae_now()  # UAE timezone - timedelta(days=7)}
        ...
        try:
            recent_penalties = await db.late_penalties.find({
                "applied_at": {"$gte": get_uae_now()  # UAE timezone - timedelta(days=30)}
...
async def create_client(
    ...
    if not client_data.client_code:
        ...
        timestamp = get_uae_now()  # UAE timezone.strftime("%y%m")
...
async def delete_client(
    ...
    client.updated_at = get_uae_now()  # UAE timezone
    db.commit()
...
async def get_credential_password(
    ...
    credential.last_used = get_uae_now()  # UAE timezone
    db.commit()
...
async def create_work_log(
    ...
    work_log = {
        ...
        "status": "open",
        "created_at": get_uae_now()  # UAE timezone,
        "updated_at": get_uae_now()  # UAE timezone
    }
...
async def update_work_log(
    ...
    update_data["updated_at"] = get_uae_now()  # UAE timezone
        ...
        for row_num in range(2, sheet.max_row + 1):
            try:
                ...
                client = Client(
                    ...
                    notes=f"تم الاستيراد من Excel في {get_uae_now()  # UAE timezone.strftime('%Y-%m-%d')}",
...
async def get_work_reports_dashboard(
    ...
    today = get_uae_now()  # UAE timezone.date().isoformat()
    ...
    week_start = (get_uae_now()  # UAE timezone.date() - timedelta(days=get_uae_now()  # UAE timezone.weekday())).isoformat()
    ...
    month_start = get_uae_now()  # UAE timezone.date().replace(day=1).isoformat()
...
async def setup_sample_data(
    ...
    try:
        ...
        for log_data in sample_logs_data:
            ...
            log_date = get_uae_now()  # UAE timezone - timedelta(days=log_data["date_offset"])
...
async def create_or_update_user_permissions(
    ...
    if permissions:
        ...
        permissions.last_updated = get_uae_now()  # UAE timezone
...
async def get_my_permissions(
    ...
    return UserPermissionResponse(
        ...
        granted_at=get_uae_now()  # UAE timezone,
        last_updated=get_uae_now()  # UAE timezone,
        is_active=True,
...
async def export_work_logs_excel(
    ...
    try:
        ...
        filename = f"work_logs_export_{get_uae_now()  # UAE timezone.strftime('%Y%m%d_%H%M')}.xlsx"
