...
```bash
# اختبار endpoint
curl -X GET "https://hr-system-upgrade.preview.emergentagent.com/api/payroll/cycles" \
...
curl -X POST "https://hr-system-upgrade.preview.emergentagent.com/api/deductions/apply-monthly" \
  -H "Authorization: Bearer {token}" \
