...
class BalanceCalculationTester:
    ...
    def get_test_employee(self) -> bool:
        ...
        if success and isinstance(response, list) and len(response) > 0:
            # Find a suitable test employee (not the super admin)
            for user in response:
                if user.get('email') != self.super_admin_creds['email'] and user.get('is_active', True):
                    self.test_employee_id = user['id']
                    self.log_test(f"Test Employee Selected: {user.get('name', 'Unknown')}", True)
                    return True
            
            # If no other employee found, use the first one
            self.test_employee_id = response[0]['id']
            self.log_test(f"Test Employee Selected: {response[0].get('name', 'Unknown')}", True)
            return True
        else:
    ...
    def create_expense_transaction(self, amount: float = 30.0) -> bool:
        ...
        # First login as the test employee to create expense
        # Get test employee credentials - we'll use a workaround by creating expense as super admin
        
        # Create a test file for the expense
        test_file_content = b"Test invoice content for balance calculation test"
        ...
        try:
            ...
            files = {'invoice_files': ('test_invoice.txt', open(test_file_path, 'rb'), 'text/plain')}
...
def main():
    ...
    backend_url = os.environ.get('REACT_APP_BACKEND_URL', 'https://visits-tracking.preview.emergentagent.com')
